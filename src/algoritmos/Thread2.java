...
/**
 * Clase Thread2: Implementa un hilo (thread) que utiliza la solución de 
 * exclusión mutua de Dekker para coordinar acceso a una región crítica en 
 * conjunto con otro hilo (probablemente Thread1).
 * 
 * @author Edson García
 */
public class Thread2 extends Thread {
    ...
    // Variables del algoritmo de Dekker
    ...
    public void Dekker(int No) {
        ...
        turno = Main.gui.getTurno(); // Obtiene el turno actual desde la interfaz gráfica
    ...
    @Override
    public void run() {
        // Ciclo infinito que se ejecuta mientras el hilo no sea interrumpido
        while (true && !this.isInterrupted()) {
            // Mientras el indicador de eventos esté activo, realiza las operaciones
            while (eventFlag) {
    ...
    private void dekker1() {
        ...
        while (Main.gui.getTurno() == 2) { // Espera su turno
            esperar();
        }
        regionCritica(2); // Accede a la región crítica
        hacerCosas("más", 2); // Realiza operaciones posteriores
        Main.gui.setTurno(2); // Indica que terminó y actualiza el turno
    }
    ...
    private void dekker2() {
        ...
        Main.gui.setP2qe(true); // Indica que quiere entrar a la región crítica
        while (Main.gui.getP1qe()) { // Espera mientras el otro hilo esté queriendo entrar
            esperar();
        }
        regionCritica(3); // Accede a la región crítica
        Main.gui.setP2qe(false); // Indica que ha terminado
        hacerCosas("más", 2); // Realiza operaciones posteriores
    }
    ...
    private void dekker3() {
        hacerCosas("", 3); // Realiza operaciones previas
        while (Main.gui.getP1qe()) { // Espera mientras el otro hilo quiera entrar
            esperar();
        }
        Main.gui.setP2qe(true); // Indica intención de acceder
    ...
    private void dekker4() {
        hacerCosas("", 2); // Realiza operaciones previas
        Main.gui.setP2qe(true); // Indica que quiere entrar a la región crítica
        while (Main.gui.getP1qe()) { // Si el otro hilo quiere entrar:
            Main.gui.setP2qe(false); // Desactiva intención de entrar
            try {
                esperar(); // Muestra que está esperando
                sleep(((int) ((Math.random() * 2) + 2)) * 1000); // Espera un tiempo aleatorio
            } catch (InterruptedException e) {}
            Main.gui.setP2qe(true); // Reactiva intención de entrar
        }
        regionCritica(4); // Accede a la región crítica
        Main.gui.setP2qe(false); // Indica que ha terminado
    ...
    private void dekker5() {
        ...
        Main.gui.setP2qe(true); // Indica que quiere entrar a la región crítica
        while (Main.gui.getP1qe()) { // Si el otro hilo quiere entrar:
            if (Main.gui.getTurno() == 1) { // Si no es su turno:
                Main.gui.setP2qe(false); // Desactiva intención de entrar
                try {
                    esperar(); // Muestra que está esperando
                    sleep(((int) ((Math.random() * 3) + 2)) * 1000); // Espera un tiempo aleatorio
                } catch (InterruptedException e) {}
                Main.gui.setP2qe(true); // Reactiva intención de entrar
            }
        }
        regionCritica(4); // Accede a la región crítica
        Main.gui.setTurno(1); // Cambia el turno al otro hilo
        Main.gui.setP2qe(false); // Indica que ha terminado
        hacerCosas("más", 3); // Realiza operaciones posteriores
    }
    ...
    private void hacerCosas(String s, int time) {
        try {
            Main.gui.setVisibleProgressBarP2("show"); // Muestra barra de progreso
            Main.gui.updateStatusP2("Haciendo " + s + " cosas..."); // Actualiza el estado
            Main.gui.setOperationProgressBarP2("working"); // Cambia operación a "trabajando"
    ...
    private void regionCritica(int time) {
        try {
            Main.gui.setVisibleProgressBarP2("show"); // Muestra barra de progreso
            Main.gui.setOperationProgressBarP2("bussy"); // Cambia operación a "ocupado"
            Main.gui.updateStatusP2("Usando región crítica..."); // Actualiza el estado
            sleep(time * 1000); // Pausa por tiempo indicado
        } catch (InterruptedException e) {}
    }
    ...
    private void esperar() {
        Main.gui.updateStatusP2("Esperando"); // Actualiza el estado a "esperando"
        Main.gui.setOperationProgressBarP2("wait"); // Cambia operación a "espera"
        Main.gui.setVisibleProgressBarP2("show"); // Muestra barra de progreso
    }
}
